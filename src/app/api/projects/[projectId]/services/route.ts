  ...
  try {
    ...
    if (q && !l) {
      query = JSON.stringify({
        query: `{ services(projectId:"${projectId}", query:"${q}", draft:false) { project { title } locations { title address } serviceId featured title shortDescription durationInfo price currency shortUrl media { url } draft } }`,
      });
    } else if (l && !q) {
      query = JSON.stringify({
